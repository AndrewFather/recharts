    ...
    if (!ifnull(show, TRUE)) {
        ...
        if (!is.null(axisPointer)) {
            ...
            lstTooltip[['axisPointer']] <- list(
                type=ifelse(type %in% c('bar', 'histogram'), 'line',
                            ifelse(type %in% c('scatter', 'line', 'point', 'bubble'),
                                   'cross', 'none')),
...
#' \code{\link{\%>\%}}.
#' @note The \code{tooltip} feature is inheritable in terms of timeliine. So, if you
#' set a timeslot-specific (z-specific) tooltip to an echarts object, you have to
#' reset the tooltip format for the following timeslot to stop the inheritability.
#' @param chart \code{echarts} object generated by \code{\link{echart}} or \code{\link{echartR}}
...
#' list(type = "line", lineStyle = list(color = "#48b", width = 2, type = "solid"),
...
#'        setTooltip(series=2, timeslots=3, bgColor='rgba(0,0,0,0.7)')
#'        # reset the following timeslot's tooltip, otherwise the series after
#'        # the 2nd one will inherit the tooltip format of the 2nd series
#' }
setTooltip <- function(chart, series=NULL, timeslots=NULL, trigger=NULL,
    ...
    if (! hasZ) {
        ...
        if (!is.null(timeslots)){
            if (all(is.numeric(timeslots))){
                vecZ <- timeslots[timeslots %in% seq_len(length(chart$x$timeline$data))]
            }else{
                vecZ <- which(chart$x$timeline$data %in% timeslots)
            }
    ...
    if (length(chartTypes) == 1) {
        ...
        if (!is.null(series)){
            if (all(is.numeric(series))){
                vecS <- series[series %in% seq_len(length(uniSeries))]
            }else{
    ...
    fixedPart <- "makeTooltip(
        ...
        show=show, formatter=determineFormatter('"
    ...
    if (identical(setAlongSZ, c(FALSE, FALSE))){  # global set
        ...
        if (hasZ) lhs <- paste0("chart$x$options[[", vecZS[,1], "]]$series[[",
                                vecZS[,2], "]][['tooltip']]")
        else lhs <- paste0("chart$x[[", vecZS[,1], "]]$series[[", vecZS[,2],
                           "]][['tooltip']]")
