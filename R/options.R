...
#' @param backgroundColor background color of plot area. Default transparent ('rgba(0,0,0,0)').
...
#' g %>% setGrid(x=40, y=30, x2=60, y2=30, backgroundColor='gray90')
#' }
...
tuneGrid <- function(chart, ...){
    ...
    if ('timeline' %in% names(chart$x)){
        ...
        lstGrid$y2 <- ifnull(lstGrid$y2, 60) + ifnull(chart$x$timeline$y2, 50) - redundY
    }else{
        lst <- chart$x
    }
    ## title
...
setTheme <- function(
    ...
    if (! is.null(backgroundColor) && backgroundColor != 'rgba(0,0,0,0)'){
        ...
        if (grepl("^rgba\\(", backgroundColor)){
            backColor <- as.numeric(unlist(strsplit(backgroundColor,
                                                    "[\\(,\\)]"))[2:5])
...
makeTooltip <- function(type, trigger=NULL, formatter=NULL,
                        ...
                        transitionDuration=0.4, backgroundColor='rgba(0,0,0,0.7)',
    ...
    if (!ifnull(show, TRUE)) {
        ...
        if (is.null(axisPointer)) {
            ...
            if (all(names(axisPointer) %in% c('show', 'type', 'lineStyle', 'crossStyle',
                                              'shadowStyle', 'textStyle')))
                lstTooltip[['axisPointer']] <- axisPointer
            else warning(paste(
                "axisPointer must be a list containing any of the below:\n",
                "'show', 'type', 'lineStyle', 'crossStyle','shadowStyle', 'textStyle'."))
        }
        ...
        if (ifnull(backgroundColor, 'rgba(0,0,0,0.7)') != 'rgba(0,0,0,0.7)')
            lstTooltip[['backgroundColor']] <- getColors(backgroundColor)[1]
    ...
    if (! 'timeline' %in% names(chart$x)) {
        ...
        if (!is.null(timeslots)){
            if (all(is.numeric(timeslots))){
                iZ <- timeslots[timeslots %in% seq_len(length(chart$x$timeline$data))]
            }else{
                iZ <- which(chart$x$timeline$data %in% timeslots)
            }
        }
    }
    if (length(types) == 1) {
        ...
        if (!is.null(series)){
            if (all(is.numeric(series))){
                iS <- series[series %in% seq_len(length(uniSeries))]
            }else{
                iS <- which(uniSeries %in% series)
            }
        }
    }
    setAlongSZ <- c(series=!is.null(series), timeslots=!is.null(timeslots))
    ## By default, do not set tooltip along series or z

    if (identical(setAlongSZ, c(FALSE, FALSE))){  # global set
        lstTooltip <- makeTooltip(
            type=types[[1]], trigger=trigger, formatter=formatter,
            islandFormatter=islandFormatter, position=position,
            enterable=enterable, axisPointer=axisPointer,
            textStyle=textStyle, showDelay=showDelay,
            hideDelay=hideDelay,
        ...
        if ('timeline' %in% names(chart$x)){
            chart$x$options[[1]][['tooltip']] <- lstTooltip
        }else{
            chart$x[['tooltip']] <- lstTooltip
        }

    }else if (identical(setAlongSZ, c(TRUE, FALSE))){  # set along series
        if ('timeline' %in% names(chart$x)){
            for (i in iS){
                chart$x$options[[1]]$series[[i]][['tooltip']] <-
                    makeTooltip(
                        type=types[i,1], trigger=trigger, formatter=formatter,
                        islandFormatter=islandFormatter, position=position,
                        enterable=enterable, axisPointer=axisPointer,
                        textStyle=textStyle, showDelay=showDelay,
                        hideDelay=hideDelay,
        ...
        if ('timeline' %in% names(chart$x)){
            for (i in iZ){
                chart$x$options[[i]][['tooltip']] <-
                    makeTooltip(
                        type=types[j,1], trigger=trigger, formatter=formatter,
                        islandFormatter=islandFormatter, position=position,
                        enterable=enterable, axisPointer=axisPointer,
                        textStyle=textStyle, showDelay=showDelay,
                        hideDelay=hideDelay,
            ...
            for (i in iZ){
                chart$x[[i]]$series[[j]][['tooltip']] <-
                    makeTooltip(
                        type=types[j,i], trigger=trigger, formatter=formatter,
                        islandFormatter=islandFormatter, position=position,
                        enterable=enterable, axisPointer=axisPointer,
                        textStyle=textStyle, showDelay=showDelay,
                        hideDelay=hideDelay,
        ...
        if ('timeline' %in% names(chart$x)){
            for (i in seq_len(length(chart$x$options))){
                for (j in iS){
                    chart$x$options[[i]]$series[[j]][['tooltip']] <-
                        makeTooltip(
                            type=types[j,i], trigger=trigger, formatter=formatter,
                            islandFormatter=islandFormatter, position=position,
                            enterable=enterable, axisPointer=axisPointer,
            ...
            for (i in seq_len(length(chart$x))){
                for (j in iS){
                    chart$x[[i]]$series[[j]][['tooltip']] <-
                        makeTooltip(
                            type=types[j,i], trigger=trigger, formatter=formatter,
