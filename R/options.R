...
setGrid <- function(chart, x=80, y=60, x2=80, y2=60, width=NULL, height=NULL,
                    backgroundColor=NULL, borderColor=NULL, borderWidth=1, ...){
    ...
    if (all(types %in% c('scatter', 'point', 'bubble', 'line', 'bar'))){
        ...
        if (! missing(backgroundColor)) if (backgroundColor != 'rgba(0,0,0,0)')
            lstGrid[['backgroundColor']] <- backgroundColor
        if (length(lstGrid) > 0){
...
makeTitle <- function(title=NULL, subtitle=NULL, link=NULL, sublink=NULL,
    ...
    if (!is.null(backgroundColor)){
        if (backgroundColor != 'rgba(0,0,0,0)')
            lstTitle[['backgroundColor']] <- backgroundColor
    }
...
#' subtitles using \%>\%.
#'
#' In echarts with timeline, you can set \code{title, subtitle, link, sublink, backgroundColor,
...
#' @param backgroundColor backgroundColor of title. Default 'rgba(0,0,0,0)' (transparent)
...
#'   subtextStyle=list(color='silver'), backgroundColor='lightgreen')
#'
#'
...
setTitle <- function(chart, title=NULL, subtitle=NULL, link=NULL, sublink=NULL,
                     pos=6, backgroundColor=NULL, borderColor=NULL, borderWidth=NULL,
    ...
    if ('timeline' %in% names(chart$x)[1]){  # has a timeline
        ...
        .dupicateThem <- function(val){
            if (!is.null(val)){
                if ((is.list(val) && is.list(val[[1]])) || is.vector(val)){
                    ...
                    if (is.list(val)) return(rep(list(val[1]), length(chart$x$options)))
                    else return(rep(val[1], length(chart$x$options)))
                }
            }else{
        ...
        backgroundColors <- .dupicateThem(backgroundColor)
        ...
        if (! all(is.null(c(titles, subtitles, links, sublinks, backgroundColor,
            ...
            for (i in seq_len(length(chart$x$options))){
                chart$x$options[[i]][['title']] <- makeTitle(
                    ...
                    sublink=sublinks[i], pos=pos, backgroundColor=backgroundColors[i],
        ...
        if (! all(is.null(c(title, subtitle, link, sublink))))
            chart$x$title <- makeTitle(
                ...
                sublink=sublink, pos=pos, borderColor=borderColor,
...
#'
#' When an echart object is generated, you can modify it by setting toolbox using \%>\%.
#' You can modify symbolList of the echarts object using this function. The symbolList
...
#' 'emptyTriangle', 'emptyDiamond')} (\strong{case sensitive}).\cr
#'  \item You can also assign non-standard symbols,
#' such as \code{'heart', 'droplet', 'pin', 'arrow', 'star5', 'star6', 'star7', 'star8'},
#' etc. (\strong{case sensitive})\cr
#'  \item When assigned to \strong{'none', no symbol is shown}, you can thus yield lines
...
getSeriesPart <- function(chart, element, ...){
    ## get variable names vector from an echarts object
    stopifnot(inherits(chart, 'echarts'))
    stopifnot(element %in% c('name', 'type', 'data', 'large'))
    if ('timeline' %in% names(chart$x)){
        obj <- sapply(chart$x$options[[1]]$series, function(lst) lst[[element]])
    }else{
...
getSeriesType <- function(chart, ...){
    ## get variable names vector from an echarts object
    stopifnot(inherits(chart, 'echarts'))
    if ('timeline' %in% names(chart$x)){
        types <- sapply(chart$x$options[[1]]$series, function(lst) lst$type)
    }else{
        types <- sapply(chart$x$series, function(lst) lst$type)
    }
...
setLegend <- function(
    ...
    series <- getSeriesPart(chart, 'name')  # get series name vector from echarts object
...
#'
#' Set theme of Echarts, including \code{color, backgroundColor, animation, calculable,
...
#' }
#' @param backgroundColor Color name/value of the background. Default is transparent
#' ('rgba(0,0,0,0)')
...
#' }
setTheme <- function(
    chart, palette='asis', backgroundColor=NULL, renderAsImage=FALSE, calculable=TRUE,
    ...
    if (! is.null(backgroundColor) && backgroundColor != 'rgba(0,0,0,0)'){
        lst[['backgroundColor']] <- ifelse(
            grepl("^rgba\\(", backgroundColor), backgroundColor,
            getColors(backgroundColor))
        if (grepl("^rgba\\(", backgroundColor)){
            backColor <- as.numeric(unlist(strsplit(backgroundColor, "[\\(,\\)]"))[2:5])
            contrastColor <- c(rep(255, 4) - backColor)
            contrastColor <- paste0('rgba(', paste(contrastColor, collapse=","), ')')
        }else if (grepl("^#", backgroundColor)){
            cColor <- c(rep(255, 4) - col2rgb(backgroundColor, TRUE))
    ...
    if ('timeline' %in% names(chart$x)) chart$x$options[[1]] <- lst
    else chart$x <- lst
