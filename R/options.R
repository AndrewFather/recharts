...
#' Set And Tune \code{grid} of Echarts (Only for Charts Of Cartesian Coordinates System)
#'
#' When an echart object is generated, you can modify it by setting and tuning grid using \%>\%.
...
#' g %>% setGrid(x=40, y=40, x2=60, y2=20, backgroundColor='lightgrey')
#' }
setGrid <- function(chart, x=80, y=60, x2=80, y2=60, width=NULL, height=NULL,
    ...
    if (all(types %in% c('scatter', 'point', 'bubble', 'line', 'bar'))){
        ...
        if ('timeline' %in% names(chart$x)){
            ...
            lstGrid$y2 <- lstGrid$y2 + chart$x$timeline$y2 - 20
        }else{
            lst <- chart$x
        }
        ## title
        if (!is.null(lst$title))
            if (lst$title$y == 'bottom'){
                if (nchar(lst$title$text) > 0)
                    lstGrid$y2 <- lstGrid$y2 + 25
                if (nchar(lst$title$subtext) > 0)
                    lstGrid$y2 <- lstGrid$y2 + 15
            }
        ## dataZoom
        if (!is.null(lst$dataZoom))
            if (lst$dataZoom$y == 'bottom'){
                lstGrid$y2 <- lstGrid$y2 + lst$dataZoom$y
            }
        ## dataRange
        if (!is.null(lst$dataRange))
            if (lst$dataRange$y == 'bottom'){
                lstGrid$y2 <- lstGrid$y2 + lst$dataRange$y
            }
